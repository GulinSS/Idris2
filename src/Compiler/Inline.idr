...
namespace EEnv
  public export
  empty : EEnv free Scope.empty
  empty = [<]

extend : EEnv free vars -> (args : List Name) -> (args' : List (CExp free)) ->
         LengthMatch args args' -> EEnv free (Scope.ext vars args)
extend env [] [] NilMatch = env
extend env (_ :: ns) (v :: vs) (ConsMatch w) = extend (env :< v) ns vs w
...
takeArgs : EEnv free vars -> List (CExp free) -> (args : Scope) ->
           Maybe (EEnv free (Scope.addInner vars args))
...
mutual
  evalLocal : {vars, free : _} ->
              ...
              EEnv free vars ->
              {idx : Nat} -> (0 p : IsVar x idx (Scope.addInner free vars)) ->
              Core (CExp free)
  ...
  evalLocal {vars = xs :< x} fc rec stk (env :< v) First
      = do log "compiler.inline.io_bind" 50 $ "Attempting to evalLocal-2, stk: \{show stk}, p: First, v: \{show v}, env: \{show @{CompileExpr} env}"
           case stk of
             [] => pure v
             _ => eval rec env stk (weakenNs (mkSizeOf xs) v)
  evalLocal {vars = xs :< x} fc rec stk (env :< _) (Later p)
      = do log "compiler.inline.io_bind" 50 $ "Attempting to evalLocal-3, stk: \{show stk}, p: Later \{show $ nameAt p}, env: \{show @{CompileExpr} env}"
  ...
  tryApply {free} {vars} rec stk env (MkFun args exp)
      = do log "compiler.inline.io_bind" 50 $ "tryApply args: \{show $ toList args}, exp: \{show exp}, stk: \{show stk}, env: \{show @{CompileExpr} env}"
  ...
  eval : {vars, free : _} ->
         ...
         EEnv free vars -> Stack free -> CExp (Scope.addInner free vars) ->
         Core (CExp free)
  eval rec env stk (CLocal fc p) =
    do log "compiler.inline.io_bind" 50 $ "Attempting to CLocal, env: \{show @{CompileExpr} env}, stk: \{show stk}, p: \{show $ nameAt p}"
  ...
  eval rec env stk (CRef fc n) = do
        log "compiler.inline.io_bind" 50 $ "Attempting to CRef, rec: \{show rec}, env: \{show @{CompileExpr} env}, stk: \{show stk}, n: \{show n}"
        ...
        case (n == NS primIONS (UN $ Basic "io_bind"), stk) of
          (True, act :: cont :: world :: stk) =>
                 do xn <- genName "act"
                    sc <- eval rec EEnv.empty [] (CApp fc cont [CRef fc xn, world])
                    pure $ unload stk $
  ...
  eval {vars} {free} rec env [] (CLam fc x sc)
      = do log "compiler.inline.io_bind" 50 $ "Attempting to CLam, rec: \{show rec}, env: \{show @{CompileExpr} env}, x: \{show x}, sc: \{show sc}"
           xn <- genName "lamv"
  ...
  eval rec env stk (CApp fc f@(CRef nfc n) args)
      = do -- If we don't know 'n' leave the arity alone, because it's
           ...
           defs <- get Ctxt
           Just gdef <- lookupCtxtExact n (gamma defs)
                | Nothing => do log "compiler.inline.io_bind" 50 $ "Attempting to CApp CRef Nothing, rec: \{show rec}, env: \{show @{CompileExpr} env}, args: \{show args}"
           ...
           log "compiler.inline.io_bind" 50 $ "Attempting to CApp CRef, rec: \{show rec}, env: \{show @{CompileExpr} env}, args: \{show args}"
           args' <- logDepth $ traverse (eval rec env []) args
           log "compiler.inline.io_bind" 50 $ "Attempting to CApp CRef, env: \{show @{CompileExpr} env}, args': \{show args'}, stk: \{show stk}, f: \{show f}"
  ...
  eval rec env stk (CForce fc lr e)
      = case !(eval rec env [] e) of
             CDelay _ _ e' => eval rec EEnv.empty stk e'
  ...
  eval rec env stk (CConCase fc sc alts def)
      = do log "compiler.inline.io_bind" 50 $ "Attempting to con case, env: \{show @{CompileExpr} env}, sc: \{show sc}"
           sc' <- logDepth $ eval rec env [] sc
           let env' = update sc env sc'
           log "compiler.inline.io_bind" 50 $ "Attempting to con case, env': \{show @{CompileExpr} env'}, stk: \{show stk}, sc': \{show sc'}, alts: \{show alts}, def: \{show def}"
           Nothing <- pickAlt rec env' stk sc' alts def | Just val => pure val
           def' <- traverseOpt (eval rec env' stk) def
           log "compiler.inline.io_bind" 50 $ "Attempting to con case, env': \{show @{CompileExpr} env'}, stk: \{show stk}, alts: \{show alts}"
    ...
    where
      updateLoc : {idx, vs : _} ->
                  (0 p : IsVar x idx (Scope.addInner free vs)) ->
      ...
      update : {vs : _} ->
               CExp (Scope.addInner free vs) -> EEnv free vs -> CExp free -> EEnv free vs
  ...
  evalConstAlt : {vars, free : _} ->
                 {auto c : Ref Ctxt Defs} ->
                 {auto l : Ref LVar Int} ->
                 List Name -> EEnv free vars -> Stack free -> CConstAlt (Scope.addInner free vars) ->
                 Core (CConstAlt free)
  ...
  pickAlt : {vars, free : _} ->
            ...
            CExp free -> List (CConAlt (Scope.addInner free vars)) ->
            Maybe (CExp (Scope.addInner free vars)) ->
            Core (Maybe (CExp free))
  ...
  pickConstAlt : {vars, free : _} ->
                 ...
                 {auto l : Ref LVar Int} ->
                 List Name -> EEnv free vars -> Stack free ->
                 CExp free -> List (CConstAlt (Scope.addInner free vars)) ->
                 Maybe (CExp (Scope.addInner free vars)) ->
                 Core (Maybe (CExp free))
...
getLams : {done : _} -> SizeOf done ->
          Int -> SubstCEnv done args -> CExp (Scope.addInner args done) ->
          (args' ** (SizeOf args', SubstCEnv args' args, CExp (Scope.addInner args args')))
...
mergeLambdas args (CLam fc x sc)
    = let (args' ** (s, env, exp')) = getLams zero 0 (Subst.empty {tm = CExp}) (CLam fc x sc)
