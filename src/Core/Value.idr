...
mutual
  ...
  {free : _} -> Show (NHead free) where
    ...
    show (NMeta n _ args) = "?" ++ show n ++ "_[" ++ show (length args) ++ " closures " ++ showClosureList (map (emptyFC,) args) ++ "]"
  ...
  export
  covering
  showClosureList : {free : _} -> SnocList (FC, Closure free) -> String
  showClosureList xs = show $ map snd xs
  ...
  {free : _} -> Show (NF free) where
    ...
    show (NBind _ x (PVTy _ c ty) _)
      = "pty " ++ showCount c ++ show x ++ " : " ++ show ty ++
        " => [closure]"
    show (NApp _ hd args) = "%NApp " ++ show hd ++ " [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
    show (NDCon _ n _ _ args) = "%NDCon " ++ show n ++ " %DCon [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
    show (NTCon _ n _ args) = "%NTCon " ++ show n ++ " %TCon [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
    show (NAs _ _ n tm) = "%NAs " ++ show n ++ "@" ++ show tm
    show (NDelayed _ _ tm) = "%Delayed " ++ show tm
    show (NDelay {}) = "%Delay [closure]"
    show (NForce _ _ tm args) = "%Force " ++ show tm ++ " [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
