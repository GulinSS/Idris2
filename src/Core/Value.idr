  ...
  {free : _} -> Show (NF free) where
    ...
    show (NBind _ x (PVTy _ c ty) _)
      = "pty " ++ showCount c ++ show x ++ " : " ++ show ty ++
        " => [closure]"
    show (NApp _ hd args) = show hd ++ " [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
    show (NDCon _ n _ _ args) = show n ++ " %DCon [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
    show (NTCon _ n _ args) = show n ++ " %TCon [" ++ show (length args) ++ " closures " ++ showClosureList args ++ "]"
