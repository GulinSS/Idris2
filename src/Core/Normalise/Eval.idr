...
evalWithOpts : {auto c : Ref Ctxt Defs} ->
               ...
               Term (Scope.addInner free vars) -> Stack free -> Core (NF free)
...
mkClosure opts locs env tm@(Local _ _ idx prf)
  ...
  where
    getLocal : {vars : _} ->
               (idx : Nat) -> (0 p : IsVar nm idx (Scope.addInner free vars)) ->
               LocalEnv free vars ->
               Maybe (Closure free)
  ...
  mutual
    ...
    eval {vars} {free} env locs (Meta fc name idx args) stk
      ...
      where
        ...
        closeArgs : List (Term (Scope.addInner free vars)) -> SnocList (Closure free)
        closeArgs [] = Scope.empty
    ...
    applyToStack env (NForce fc r tm args) stk
       = do tm' <- applyToStack env tm []
            case tm' of
                 ...
                 _ => pure (NForce fc r tm' (Scope.ext args stk))
    ...
    evalLocal env fc mrig (S idx) (Later p) stk (locs :< _)
    ...
    evalRef env meta fc (TyCon arity) fn stk def
        = do -- logC "eval.ref.type" 50 $ do fn' <- toFullNames fn
             ...
             pure $ ntCon fc fn arity (cast stk)
    ...
    argsFromStack [<] stk = Just (LocalEnv.empty, stk)
    ...
    argsFromStack (ns :< n) (arg :: args)
        ...
        where
            -- TODO: Find a way for better performance
...
nf defs env tm = logDepth $ eval defs defaultOpts env LocalEnv.empty tm []

export
nfOpts : {auto c : Ref Ctxt Defs} ->
         {vars : _} ->
         EvalOpts -> Defs -> Env Term vars -> Term vars -> Core (NF vars)
nfOpts opts defs env tm = logDepth $ eval defs opts env LocalEnv.empty tm []
